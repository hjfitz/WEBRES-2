...
let carModel; let controls;let myCarRef
...
function update() {
  ...
  if (multiplayer.private) {
    multiplayer.players.map((p) => {
      let model = scene.getObjectByName(p.id)
  ...
  stats.update();
  if (myCarRef) {
    multiplayer.sendPosition(multiplayer.socketId,{x:myCarRef.position.x,y:myCarRef.position.y,z:myCarRef.position.z})
  }
...
async function init(useVR = false) {
  ...
  multiplayer.players.map( async (player) => {
    if (player.id != multiplayer.socketId) {
      await initCar(useVR, false, player.id)
    }
  })
