...
// use node require so that THREE isn't immurable
const THREE = require('./three');
const THREEx = require('./three/THREEx.FullScreen');
require('./three/PointerLockControls');
require('./three/DRACOLoader');
require('./three/GLTFLoader');

const Stats = require('./three/stats.min');
const WEBGL = require('./three/WebGL');
const WEBVR = require('./three/WebVR');
 ...
 *
 * @param {boolean} useVR
 * @param {boolean} isOtherPlayer
 * @param {string} socketId
 */
async function initCar(useVR, isOtherPlayer, socketId) {
  // car config
  const car = new Car(isOtherPlayer);
...
function pollGamePad() {
  ...
  const [jsLeft] = pad.axes; // left joystick, left and right
...
function renderGame(delta) {
  ...
  if (myCarRef) {
    ...
    multiplayer.sendMovement(multiplayer.socketId, myCarRef.getCarVars());
  }
  renderer.render(scene, camera);
}

function updateGameVR() {
...
async function init(useVR = false) {
  ...
  if (document.fullscreenEnabled) {
    document.body.addEventListener('keydown', (ev) => {
      if (ev.keyCode === 70) THREEx.FullScreen.request();
    }, false);
  }


  // can't use an arrow function here as we need to bind to Multiplayer object
  multiplayer.onReady(() => {
    myCarRef.root.position.x = 0;
    myCarRef.root.position.z = 0;
  }, true);

  // renderer.setAnimationLoop(update);
  if (!useVR) updateGame();
  else renderer.setAnimationLoop(updateGameVR);
}
